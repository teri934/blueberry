...
namespace Dictionary.Implemented
{
	class SearchViewListener : Java.Lang.Object, SearchView.IOnQueryTextListener
	{
		...
		{
			RecordingAdapter adapter = (RecordingAdapter)RecordingsFragment.myList.Adapter;
			RecordingFilter filter = (RecordingFilter)adapter.Filter;
	...
	class RecordingAdapter : BaseAdapter, IFilterable
	{
		...
		public RecordingAdapter(Android.Content.Context context, List<Word> data)
		{
			originalData = data;
			filteredData = data;
		...
		public Filter Filter => new RecordingFilter();
